...
class SeoController extends Controller
{
    public function dashboard()
    {
        ...
        $redirects=RedirectRule::count(); $fours=PageViewLog::where('hit_count','>=',1)->count();
    ...
    public function pages()
    {
        ...
        foreach([['/','Home'],['/about-us','About Us'],['/commercial-cleaning','Commercial Cleaning'],['/services','Services'],['/contact','Contact'],['/locations','Locations'],['/case-studies','Case Studies'],['/book-walkthrough','Book Walkthrough']] as [$slug,$title]) {
            if(!$pages->firstWhere('slug',$slug)) $pages->push(SeoMeta::create(['slug'=>$slug,'page_title'=>$title,'page_type'=>'page','is_active'=>true,'robots_index'=>true,'robots_follow'=>true]));
        }
    ...
    public function updatePage(Request $request,SeoMeta $seoMeta)
    {
        ...
        $data['robots_index']=$request->boolean('robots_index',true); $data['robots_follow']=$request->boolean('robots_follow',true);
    ...
    public function updateBlog(Request $request,BlogPost $blogPost)
    {
        ...
        $data['page_type']='post'; unset($data['image_alt_text']);
    ...
    {
        $data=$request->validate([
            'schema_organization'=>'nullable|string','schema_website'=>'nullable|string','schema_local_business'=>'nullable|string',
            'schema_service'=>'nullable|string','schema_product'=>'nullable|string','schema_breadcrumb'=>'nullable|string','schema_custom'=>'nullable|string',
        ]);
        foreach($data as $key=>$value){ if($value!==null && $value!==''){ json_decode($value,true); if(json_last_error()!==JSON_ERROR_NONE) return back()->withErrors([$key=>'Each schema field must contain valid JSON-LD JSON.'])->withInput(); } }
        $settings=$this->settings(); $settings->fill($data)->save();
    ...
    {
        $data=$request->validate(['robots_txt'=>'required|string']);
        $settings=$this->settings(); $settings->update($data);
    ...
    public function sitemap()
    {
        ...
        foreach(BlogPost::published()->get() as $p) $urls[]=['loc'=>route('blog.show',$p),'changefreq'=>'weekly','priority'=>'0.7'];
    ...
    public function robots(){ $settings=$this->settings(); $content=$settings->robots_txt ?: "User-agent: *\nDisallow: /admin\nSitemap: ".url('/sitemap.xml')."\n"; return response($content,200)->header('Content-Type','text/plain'); }
    private function settings(): SeoSetting{return SeoSetting::firstOrCreate([],['site_name'=>config('app.name'),'default_title'=>config('app.name'),'default_description'=>'Professional cleaning services.']);}
}
